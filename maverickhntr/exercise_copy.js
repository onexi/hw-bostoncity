...
exercise.earningsAbove = function(target) {
    ...
    */
    ...
    var baseEarnings = people.map(exercise.getEarnings);
    ...
    var num_salaraies = 0;
    var aboveEarnings = baseEarnings.filter(isBigEnough);
...
exercise.totalBasePayroll = function() {
    ...
    */
    var people = exercise.data.data; // get handle on data
    var baseEarnings = people.map(exercise.getEarnings);
    var totalPayroll = baseEarnings.reduce(function(a,b) {
        return a + b;
    });
    return totalPayroll;
...
exercise.totalEarningsWithOvertime = function() {
    ...
    */
    var people = exercise.data.data; // get handle on data
    console.log(people);
    var baseEarnings = people.map(function(x){
        var personEarnings = x[12];
        return personEarnings;
    });
